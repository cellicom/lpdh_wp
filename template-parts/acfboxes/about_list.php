    ...
    <?php if ($people): ?>
        <div class="row g-4">
            <?php foreach ($people as $person): 
                if (!($person['visible'] ?? true)) continue;
                ...
                if ($profile) {
                    if (!$title) $title = $profile['display_name'];
                    if (!$nickname) $nickname = $profile['user_nicename'];
                    if (!$icon) $icon = get_avatar_url($profile['ID']);
                }
            ...
            ?>
                <div class="col-12">
                    <?php if ($has_link): ?>
                    <a href="<?php echo esc_url($href); ?>" target="<?php echo esc_attr($target); ?>" class="text-decoration-none">
                    <?php endif; ?>
                                ...
                                <div class="d-flex align-items-start gap-4">
                                    <?php if ($icon): ?>
                                        <div class="flex-shrink-0">
                                            <img src="<?php echo esc_url($icon); ?>" alt="" class="rounded-circle" style="height: 80px; width: 80px; object-fit: cover;">
                                        </div>
                                    <?php endif; ?>
                                    <div class="flex-grow-1">
                                        <div class="d-flex align-items-center gap-2 mb-1">
                                            ...
                                            <?php endif; ?>
                                            <?php if ($nickname): ?>
                                                <span class="badge bg-light text-dark border">@<?php echo esc_html($nickname); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <?php if ($subtitle): ?>
