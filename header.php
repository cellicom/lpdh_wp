...
<body <?php body_class(); ?>>
  ...
  <div id="page" class="site">
    ...
    <a class="skip-link visually-hidden-focusable"
      href="#footer"><?php esc_html_e('Skip to footer', 'bootscore'); ?></a>
    ...
    <header id="masthead"
      class="<?= esc_attr(apply_filters('bootscore/class/header', 'sticky-top bg-body-tertiary')); ?> site-header">

      <?php do_action('bootscore_after_masthead_open'); ?>

      <nav id="nav-main"
        class="navbar <?= esc_attr(apply_filters('bootscore/class/header/navbar/breakpoint', 'navbar-expand-lg')); ?>">
        ...
        <div class="<?= esc_attr(apply_filters('bootscore/class/container', 'container', 'header')); ?>">
          ...
          <!-- Offcanvas Navbar -->
          <div
            class="offcanvas offcanvas-<?= esc_attr(apply_filters('bootscore/class/header/offcanvas/direction', 'end', 'menu')); ?>"
            tabindex="-1" id="offcanvas-navbar">
            <div
              class="offcanvas-header <?= esc_attr(apply_filters('bootscore/class/offcanvas/header', '', 'menu')); ?>">
              <span
                class="h5 offcanvas-title"><?= esc_html(apply_filters('bootscore/offcanvas/navbar/title', __('Menu', 'bootscore'))); ?></span>
          ...
          <div
            class="header-actions <?= esc_attr(apply_filters('bootscore/class/header-actions', 'd-flex align-items-center')); ?>">
            ...
            <!-- Navbar Toggler -->
            <button
              class="<?= esc_attr(apply_filters('bootscore/class/header/button', 'btn btn-outline-secondary', 'nav-toggler')); ?> <?= esc_attr(apply_filters('bootscore/class/header/navbar/toggler/breakpoint', 'd-lg-none')); ?> <?= esc_attr(apply_filters('bootscore/class/header/action/spacer', 'ms-1 ms-md-2', 'nav-toggler')); ?> nav-toggler"
              type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvas-navbar"
              aria-controls="offcanvas-navbar" aria-label="<?php esc_attr_e('Toggle main menu', 'bootscore'); ?>">
              <?= wp_kses_post(apply_filters('bootscore/icon/menu', '<i class="fa-solid fa-bars"></i>')); ?> <span
